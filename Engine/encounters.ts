...
//   datalayer.ts  — DataStore, Loader
//   skills.ts     — getSkillLevel
//
...
import { DataStore } from "./datalayer.js";
import { getSkillLevel } from "./skills.js";

// The Loader is threaded through as a parameter (its concrete type is the
// datalayer Loader export); these helpers only call `.load`.
...
export const PartySkills = (() => {
  ...
  const getInsts = (save: any, Loader: LoaderLike): any[] =>
    save.party
      .map((m: any) => {
  ...
  const partyHasProfession = (insts: any[], prof: string): boolean =>
  ...
  // Uses getSkillLevel (skills.ts) so both the {level,xp} object form and the
  // legacy flat-number form resolve correctly.
  const partySkillLevel = (insts: any[], skill: string): number =>
    insts.reduce((best, i) => {
      if (i.deathState !== "alive") return best;
      const lvl = getSkillLevel(i, skill);
...
export const checkReroll = (slot: any, save: any, table: any, partyInsts: any[], Loader: LoaderLike): boolean => {
  const cond = slot.rerollIf;
  if (!cond) return false;

  // ── companion availability ──────────────────────────────────────────────
  if (cond === "no_eligible_companions") {
    const existing = save.party.map((p: any) => p.templateId);
    const pool     = table.recruitPool || [];
    return !pool.some((id: string) => {
  ...
  if (cond === "no_eligible_quests") {
    ...
    const zoneQuests  = DataStore.list("templates/quests/").filter((p: string) => {
...
export const EncounterGenerator = (() => {
  ...
  const buildRollTable = (slots: any[], save: any): any[] => {
  ...
  // ── weighted slot roll ────────────────────────────────────────────────────
  const rollSlot = (entries: any[]): any => {
    let r = Math.random();
    for (const e of entries) {
      r -= e._chance;
      if (r <= 0) return e;
  ...
  const rollCombatGroup = (table: any, partySize: number, save: any, Loader: LoaderLike): { ok: boolean; enemies?: any[]; errors?: string[] } => {
    ...
    if (activeTrack) {
      const matching = pool.filter((entry: any) => {
    ...
    const boost    = save.flags?.trackingBoost || null;
    const weighted = filteredPool.map((e: any) => {
      let w = e.weight || 1;
      if (boost && boost.targetId === e.enemyId) w *= (boost.multiplier || 1.5);
      return { ...e, _w: w };
    });
    const totalW = weighted.reduce((s: number, e: any) => s + e._w, 0);

    const pickWeighted = (pool: any[], total: number): any => {
    ...
    let allowedIds: string[] | null = null; // null = all enemies eligible to co-spawn
    ...
    const eligible      = allowedIds ? weighted.filter((e: any) => allowedIds!.includes(e.enemyId)) : weighted;
    const eligibleTotal = eligible.reduce((s: number, e: any) => s + e._w, 0);
  ...
  const generate = (zoneId: string, save: any, Loader: LoaderLike, _depth = 0): any => {
    if (_depth > MAX_DEPTH) {
    ...
    if (slot.type === "companion") {
      const existing = save.party.map((p: any) => p.templateId);
      const pool     = table.recruitPool || [];
      const avail    = pool.filter((id: string) => {
        if (existing.includes(id)) return false;
    ...
    if (slot.type === "quest") {
      const assignedIds = Object.keys(save.quests || {});
      const zonePaths   = DataStore.list("templates/quests/").filter((p: string) => {
        const r = DataStore.read(p);
        if (!r || r.zoneId !== zoneId || assignedIds.includes(r.id)) return false;
        if ((r.prerequisites || []).some((pid: string) => !save.quests?.[pid]?.completed)) return false;
...
export const runEncounterTests = (DataStore: any, Loader: LoaderLike, _save?: any): TestRun => {
  ...
  const nodeSlot  = etData.slots?.find((sl: any) => sl.nodeId === "template_node");
  const chestSlot = etData.slots?.find((sl: any) => sl.type   === "locked_chest");
  const fishSlot  = etData.slots?.find((sl: any) => sl.type   === "fishing_spot");
  ...
  for (let i = 0; i < 100; i++) {
    const e = EncounterGenerator.generate("colonial_sewers", save, Loader);
    if (e.encounterType === "gathering" && e.gatheringNodes?.some((n: any) => (n.nodeId || n.itemId) === "template_node")) gotNode = true;
  }
  assert("generate: mining node can surface with miner", gotNode);

  // ── combat group ────────────────────────────────────────────────────────
  let combatEnc: any = null;
  for (let i = 0; i < 50; i++) {
    const e = EncounterGenerator.generate("colonial_sewers", save, Loader);
  ...
  if (combatEnc) {
    ...
    assert("combat: each enemy has name",            combatEnc.enemies.every((e: any) => !!e.name));
    assert("combat: each enemy has instanceId",      combatEnc.enemies.every((e: any) => !!e.instanceId));
  } else {
...
export const reportEncounterTests = (r: TestRun): string => {
  const lines = [
    `\n${"=".repeat(60)}`,
