...
//
// DEPENDENCIES: Data/item_suffixes.json + DataStore/Loader (Engine/datalayer.ts).
// =============================================================================

import { DataStore, Loader } from "./datalayer.js";

type BaseItem = Record<string, any>;

interface SuffixGroup { suffix: string; stats: string[]; }
interface SuffixData {
  suffixGroups: Record<string, SuffixGroup>;
  statCost: Record<string, number>;
  slotMultipliers: Record<string, number>;
  rollChance: number;
  craftRollChance?: number;
  valueMultiplier: number;
  upgradedQuality: string;
}

const _suffixData = require("../Data/item_suffixes.json") as SuffixData;
...
export const ItemSuffixes = (() => {
  ...
  const greenBudget = (itemLevel: number | null | undefined): number => Math.max(2, Math.round(0.5 * (itemLevel || 1) - 2));

  // How much of the budget a given item slot/weapon type gets to spend.
  const slotMultiplier = (item: BaseItem): number => {
    if (item.type === "weapon") {
  ...
  const computeStatBonuses = (baseItem: BaseItem, suffixKey: string): Record<string, number> => {
  ...
  const upgradeQuality = (quality: string): string => {
  ...
  const isEligibleBaseItem = (item: BaseItem | null | undefined): boolean =>
    !!item && Array.isArray(item.tags) && item.tags.includes("randomEnchant");

  // Builds the synthetic suffixed-item definition for one (baseItem, suffixKey) pair.
  const buildVariant = (baseItem: BaseItem, suffixKey: string): BaseItem => {
    const suffix = SUFFIX_GROUPS[suffixKey];
  ...
  const generateAllVariants = (itemsMap: Record<string, BaseItem> | null | undefined): BaseItem[] => {
    const out: BaseItem[] = [];
  ...
  const maybeApplySuffix = (itemId: string, chance = ROLL_CHANCE): string => {
  ...
  const isEligible = (itemId: string): boolean => {
    const ir = Loader.load(`templates/items/${itemId}`, "item");
...
export const runSuffixTests = (): TestRun => {
...
export const reportSuffixTests = (r: TestRun): string => {
  const lines = [
    `\n${"=".repeat(60)}`,
