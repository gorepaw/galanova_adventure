...
exports.onStoryWipe = exports.onStoryBossDefeated = exports.onStoryCombatVictory = exports.applyEffects = exports.advanceScene = exports.resolvePendingScene = exports.enqueueScene = exports.hasPendingScene = void 0;
const datalayer_js_1 = require("./datalayer.js");
const loadDialogue = (id) => {
    const r = datalayer_js_1.Loader.load(`templates/dialogues/${id}`, "dialogue");
    return r.ok ? r.data : null;
};
const loadSpeaker = (id) => {
    if (!id)
        return null;
    const r = datalayer_js_1.Loader.load(`templates/speakers/${id}`, "speaker");
    return r.ok ? r.data : null;
};
const findNode = (dlg, nodeId) => (dlg?.nodes || []).find((n) => n.id === nodeId) || null;
const hasPendingScene = (save) => Array.isArray(save?.sceneQueue) && save.sceneQueue.length > 0;
exports.hasPendingScene = hasPendingScene;
...
const enqueueScene = (save, dialogueId, opts = {}) => {
    const dlg = loadDialogue(dialogueId);
    if (!dlg || !dlg.nodes?.length)
        return save;
    const seen = save.seenScenes || [];
    const queue = save.sceneQueue || [];
    if (!opts.force && seen.includes(dialogueId))
        return save;
    if (queue.some(q => q.dialogueId === dialogueId))
        return save;
    return { ...save, sceneQueue: [...queue, { dialogueId, nodeId: dlg.nodes[0].id }] };
};
exports.enqueueScene = enqueueScene;
// Resolve the head of the queue into a UI view-model, or null if none.
const resolvePendingScene = (save) => {
    if (!(0, exports.hasPendingScene)(save))
        return null;
    const head = save.sceneQueue[0];
    ...
    const choices = (node.choices || []).map((c, i) => ({ index: i, label: c.label }));
...
exports.resolvePendingScene = resolvePendingScene;
...
const advanceScene = (save, choiceIndex) => {
    if (!(0, exports.hasPendingScene)(save))
        return { save, effects: [], sceneEnded: false };
    const queue = save.sceneQueue.slice();
    const head = queue[0];
    const dlg = loadDialogue(head.dialogueId);
...
const assignQuest = (save, questId) => {
    if (!questId || save.quests?.[questId])
        return save;
    const r = datalayer_js_1.Loader.load(`templates/quests/${questId}`, "quest");
    const objectives = {};
    if (r.ok)
        for (const obj of (r.data.objectives || []))
            objectives[obj.id] = 0;
    return {
        ...save,
        quests: {
...
const applyReward = (save, eff, log) => {
    let s = save;
    if (eff.currency) {
        s = { ...s, currency: (s.currency || 0) + eff.currency };
        log.push(`   Reward: +${eff.currency}c`);
    }
    for (const it of (eff.items || [])) {
        s = datalayer_js_1.Modifiers.addToInventory(s, it.itemId, it.qty || 1);
...
const applyEffects = (save, effects) => {
    let s = save;
    const deferred = [];
    const log = [];
    for (const eff of effects || []) {
        switch (eff.type) {
            case "setFlag":
                s = datalayer_js_1.Modifiers.setFlag(s, eff.flag, eff.value ?? true);
                break;
            case "assignQuest":
                s = assignQuest(s, eff.questId);
                break;
            case "completeQuest":
                s = datalayer_js_1.Modifiers.completeQuest(s, eff.questId);
                log.push(`   ✓ Quest complete: ${eff.questId}`);
                break;
            case "giveReward":
                s = applyReward(s, eff, log);
                break;
            case "startCombat":
            case "travel":
                deferred.push(eff);
...
function* activeStoryQuests(save) {
    for (const [questId, qstate] of Object.entries(save.quests || {})) {
        if (qstate?.completed)
            continue;
        const r = datalayer_js_1.Loader.load(`templates/quests/${questId}`, "quest");
        if (!r.ok || r.data.type !== "story")
            continue;
        if (r.data.zoneId && save.currentZone !== r.data.zoneId)
            continue;
...
const onStoryCombatVictory = (save) => {
    let s = save;
    for (const [, , quest] of activeStoryQuests(s)) {
        ...
        if (bossThreshold && next === bossThreshold && script.bossScene) {
            s = (0, exports.enqueueScene)(s, script.bossScene, { force: true });
            continue;
        }
        for (const stage of (quest.onStage || [])) {
            if (stage.at === next && stage.dialogueId)
                s = (0, exports.enqueueScene)(s, stage.dialogueId);
        }
    }
    return s;
};
exports.onStoryCombatVictory = onStoryCombatVictory;
...
const onStoryBossDefeated = (save, kills = []) => {
    let s = save;
    let defeated = false;
    const killedIds = new Set((kills || []).map((k) => k?.id).filter(Boolean));
    for (const [, , quest] of activeStoryQuests(s)) {
        ...
        if (bossId && killedIds.has(bossId)) {
            ...
            if (victory)
                s = (0, exports.enqueueScene)(s, victory, { force: true });
        }
    }
    return { save: s, defeated };
};
exports.onStoryBossDefeated = onStoryBossDefeated;
...
const onStoryWipe = (save, wasBoss) => {
    let s = save;
    for (const [, , quest] of activeStoryQuests(s)) {
        const script = quest.script || {};
        const counterKey = script.counter || `${quest.id}_wins`;
        ...
        if (scene)
            s = (0, exports.enqueueScene)(s, scene, { force: true });
    }
    return s;
};
exports.onStoryWipe = onStoryWipe;
