...
//
// DEPENDENCIES:
//   data_layer.js   — DataStore, Loader, Modifiers
//   encounter.js    — EncounterGenerator (patched below), PartySkills
//   game_loop.js    — CombatBridge, RewardEngine, DeathHandler
...
const DungeonManager = (() => {
  ...
  const getCurrentEncounter = (save, Loader) => {
...
const DungeonEncounterBuilder = (() => {
  ...
  const buildEnemyList = (enemyIds, Loader) => {
    const enemies = [];
  ...
  const buildCombatEncounter = (entry, zoneId, Loader) => ({
    ok:               true,
    zoneId,
  ...
  const build = (entry, zoneId, Loader) => {
...
const patchEncounterGenerator = () => {
  const _original = EncounterGenerator.generate;

  EncounterGenerator.generate = (zoneId, save, Loader, _depth = 0) => {
...
const patchRewardEngine = () => {
  const _originalApply = RewardEngine.apply;

  RewardEngine.apply = (combatResult, encounter, save) => {
...
const DungeonUI = (() => {
  ...
  const renderTextEncounter = (encounter) => {
    const lines = [];
  ...
  const renderBossOutro = (encounter) => {
    const lines = [];
  ...
  const renderDungeonComplete = (zoneId, Loader) => {
    const zr = Loader.load(`templates/zones/${zoneId}`, "zone");
...
const runDungeonTests = (DataStore, Loader) => {
...
if (typeof module !== "undefined") {
  module.exports = {
    DUNGEON_SCHEMA_PATCHES,
    DungeonManager,
    DungeonEncounterBuilder,
    DungeonUI,
    patchEncounterGenerator,
    patchRewardEngine,
    EXAMPLE_DUNGEON_ZONE,
    EXAMPLE_WING_A,
    runDungeonTests,
    reportDungeonTests,
  };
